...
# -*- coding: utf-8 -*-
"""
BF16-friendly test script for trained SCT TSP model (single-process / DDP-friendly data path)
...
- Runs model forward under CUDA autocast(bfloat16), but keeps discrete post-processing in fp32
"""
...
# Use the bf16-safe model that handles fp32 fallbacks internally
...
def load_model(model_path: str, device: torch.device, output_dim: int) -> tuple[torch.nn.Module, SimpleNamespace]:
    ...
    model = GNN(
        ...
        inference_mode=True,
        # Model internally guards numerically sensitive ops with fp32
        # and respects autocast outside; no need to pass amp flags here.
    ).to(device)
...
def evaluate_model(
    ...
    Forward runs under autocast(bfloat16) when enabled; post-processing stays float32.
    """
    model.eval()
    ...
    # Autocast control for CUDA bfloat16
    use_amp = (device.type == "cuda") and use_bf16 and bf16_supported()
    if use_amp:
        print("→ Using CUDA autocast(dtype=bfloat16) for inference.")
    else:
        print("→ Running inference in float32 (autocast disabled).")
    ...
    for bidx, coordinates_batch in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        # Pairwise distances in fp32 for stable lengths
        dist_mat = torch.cdist(coordinates_batch.float(), coordinates_batch.float())
        adj = torch.exp(-dist_mat / float(distance_scale))

        # Forward (inference returns logits). Autocast region only around forward.
        t0 = time.time()
        if use_amp:
            ...
            logits = model(coordinates_batch, adj)
        # Discrete / loss computations in fp32
        logits_f32 = logits.float()
        P_hard = to_exact_permutation_batched(logits_f32)
        inference_times.append(time.time() - t0)

        # Tour lengths via the same loss utility (fp32)
        ...
        total_instances += coordinates_batch.shape[0]
...
def main():
    ap = argparse.ArgumentParser(description="Test trained SCT TSP model (bf16-friendly)")
    ...
    ap.add_argument("--save_dir", type=str, default="test_results_bf16")
    # precision toggles
    ap.add_argument("--no_bf16", action="store_true", help="Disable autocast(bfloat16) and run in fp32")
    args = ap.parse_args()

    # Basic CUDA perf knobs
    torch.backends.cuda.matmul.allow_tf32 = True  # allow TF32 on matmul (A100/H100)
    ...
    try:
        from tsp_visualization import visualize_batch_tours  # guarded import
        VIZ_AVAILABLE = True
    except Exception:
        VIZ_AVAILABLE = False
    ...
    with open(summary_path, "w") as f:
        f.write("SCT TSP Model Test Results (bf16)\n")
        f.write("=" * 50 + "\n\n")
