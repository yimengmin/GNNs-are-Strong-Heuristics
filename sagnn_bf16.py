...
#
#    @staticmethod
#    def _safe_eigh_2x2(cov_fp32: torch.Tensor):
#        # Robust eigendecomposition in fp32
#        eigvals, eigvecs = torch.linalg.eigh(cov_fp32)  # [B,2],[B,2,2]
...
#        dtype_orig = coords.dtype
#        coords32 = coords.to(torch.float32)
#        centers32 = coords32.mean(dim=1, keepdim=True)
#        centered32 = coords32 - centers32
#        cov32 = torch.matmul(centered32.transpose(1, 2), centered32) / coords32.shape[1]  # [B,2,2]
#        # small ridge for degeneracy
#        cov32 = cov32 + torch.eye(2, device=cov32.device, dtype=cov32.dtype).unsqueeze(0) * 1e-8
#        u32, uperp32 = self._safe_eigh_2x2(cov32)
#        centers = centers32.squeeze(1).to(dtype_orig)
#        u = u32.to(dtype_orig); u_perp = uperp32.to(dtype_orig)
#        return centers, u, u_perp
#
#    def _polar_feats(self, coords, c, u, u_perp):
#        x = coords - c.unsqueeze(1)         # [B,N,2]
#        ax = (x * u.unsqueeze(1)).sum(-1)
#        ay = (x * u_perp.unsqueeze(1)).sum(-1)
#        # do radius/theta in fp32 for numerical stability then cast back
#        ax32 = ax.to(torch.float32); ay32 = ay.to(torch.float32)
#        r32 = torch.sqrt(ax32.pow(2) + ay32.pow(2) + 1e-6)
#        theta32 = torch.atan2(ay32, ax32)
#        m = torch.arange(1, self.phi_harmonics + 1, device=coords.device, dtype=torch.float32)
#        thm32 = theta32.unsqueeze(-1) * m.unsqueeze(0).unsqueeze(0)
#        feats = torch.cat([
#            r32.unsqueeze(-1),
#            ax32.unsqueeze(-1),
#            ay32.unsqueeze(-1),
...
#        # compute stats in fp32, then cast back
#        coords32 = coords.to(torch.float32)
#        mean_coords = coords32.mean(dim=1)
#        std_coords = coords32.std(dim=1, unbiased=False).clamp_min(1e-6)
#        min_coords = coords32.min(dim=1)[0]
#        max_coords = coords32.max(dim=1)[0]
#        bbox_size = (max_coords - min_coords).clamp_min(1e-6)
#        bbox_center = (max_coords + min_coords) / 2
#        gs = torch.cat([mean_coords, std_coords, bbox_size, bbox_center], dim=-1)
#        return gs.to(coords.dtype)
#
...
class EquivariantCoordinateFeatureExtractor(nn.Module):
    ...
    @staticmethod
    def _safe_eigh_2x2(cov: torch.Tensor):
        """
        cov: [B, 2, 2], possibly under autocast(bf16). We upcast and
        disable autocast only for eigh to avoid CUDA 'not implemented' errors.
        """
        # Always run eigh in float32 on the same device
        cov_fp32 = cov.to(torch.float32)

        # Disable autocast just for eigh
        if cov_fp32.is_cuda:
        ...
        # principal (largest eigenvalue) is the last vector
        u32      = eigvecs[:, :, -1]      # [B,2]
        uperp32  = eigvecs[:, :,  0]      # [B,2]
        return u32, uperp32

    def _principal_axes(self, coords: torch.Tensor):
        # Work in float32 for stability; coords may be bf16 under autocast
        coords32  = coords.to(torch.float32)
        centers32 = coords32.mean(dim=1, keepdim=True)          # [B,1,2]
        centered  = coords32 - centers32                         # [B,N,2]
        cov32     = torch.matmul(centered.transpose(1, 2), centered) / coords32.shape[1]  # [B,2,2]

        u32, uperp32 = self._safe_eigh_2x2(cov32)

        # Deterministic sign (first component non-negative)
        sign = torch.where(u32[:, 0] < 0, -1.0, 1.0).unsqueeze(-1)
        u32     = u32 * sign
        uperp32 = uperp32 * sign

        # Return centers in fp32 as well (caller can cast later if needed)
        return centers32.squeeze(1), u32, uperp32

    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        # coords might be bf16 here; internals upcast where needed
        c32, u32, uperp32 = self._principal_axes(coords)  # all fp32
        # Use fp32 for downstream polar features, then you can cast back if needed
        B, N, _ = coords.shape
        coords32 = coords.to(torch.float32)

        x   = coords32 - c32.unsqueeze(1)                 # [B,N,2]
        ax  = (x * u32.unsqueeze(1)).sum(-1)
        ay  = (x * uperp32.unsqueeze(1)).sum(-1)
        r   = torch.sqrt(ax.pow(2) + ay.pow(2) + 1e-6)
        th  = torch.atan2(ay, ax)

        m = torch.arange(1, self.phi_harmonics + 1, device=coords32.device, dtype=coords32.dtype)
        thm = th.unsqueeze(-1) * m.unsqueeze(0).unsqueeze(0)

        polar_feats = torch.cat([r.unsqueeze(-1),
                                 ax.unsqueeze(-1),
                                 ay.unsqueeze(-1),
                                 torch.sin(thm),
                                 torch.cos(thm)], dim=-1)
        ...
        if self.include_global_stats:
            ...
            bbox_center = (max_coords + min_coords) / 2
            gs = torch.cat([mean_coords, std_coords, bbox_size, bbox_center], dim=-1)
            gs = gs.unsqueeze(1).expand(-1, N, -1)
            feats = torch.cat([polar_feats, gs], dim=-1)
        else:
            feats = polar_feats

        # If the rest of the model expects the incoming dtype (e.g., bf16 under autocast),
        # cast back to coords.dtype on return:
...
def precompute_graph_operators(W: torch.Tensor, gcn_order: int, sct_order: int) -> Dict[str, List[torch.Tensor]]:
    """
    Build GCN and SCT operators in fp32 for stability, then cast back to W.dtype.
    """
    B, N, _ = W.shape
    device = W.device
    dt = W.dtype
    ...
    for _ in range(1, max(gcn_order, 1)):
        gcn_ops32.append(torch.bmm(gcn_ops32[-1], A_norm))

    # Row-stochastic P and S = 0.5(I + P)
    deg_row = W32.sum(-1, keepdim=True).clamp_min(1e-6)  # [B,N,1]
    P = W32 / deg_row                                    # row-stochastic (rows sum to 1)

    I = torch.eye(N, device=device, dtype=W32.dtype).unsqueeze(0).expand(B, -1, -1)
    S = 0.5 * (I + P)

    # S powers: S, S^2, S^4, ...
    S_powers32 = [S]
    for _ in range(sct_order):
        S_powers32.append(torch.bmm(S_powers32[-1], S_powers32[-1]))

    # SCT operators as differences
    sct_ops32 = []
    for i in range(sct_order):
        M_i = S_powers32[i] - S_powers32[i + 1]
        sct_ops32.append(M_i)

    gcn_ops = [t.to(dt) for t in (gcn_ops32 if gcn_order > 0 else [])]
    sct_ops = [t.to(dt) for t in (sct_ops32 if sct_order > 0 else [])]
    return {"gcn": gcn_ops, "sct": sct_ops}

# ------------------- Attention-mixing layer -------------------
class FastSCTLayer(nn.Module):
    ...
    def forward(self, X, diffusion_outs: List[torch.Tensor]):
        ...
        # temperature scaling is optional but okay
...
def fast_sinkhorn_iteration(Z, n_iter=10):
    # Sinkhorn always in fp32 for stability
...
def fast_gumbel_sinkhorn(logits, tau=0.5, n_iter=10, noise_scale=0.05, clamp_val=20.0):
    # logits can be fp16, but we upcast for the noisy Sinkhorn process
    if noise_scale > 0:
    ...
    Z_init = (logits.to(torch.float32).clamp(-clamp_val, clamp_val) + g) / max(tau, 1e-3)
...
# ------------------- EnhancedUltraFastSCTGNN (AMP-ready / fp16-safe) -------------------
class EnhancedUltraFastSCTGNN(nn.Module):
    def __init__(
        ...
        amp_autocast: bool = False,           # enable AMP outside + inside forward
    ...
    def forward(self, X, adj, moment: float = 1.0, cache: Optional[Dict] = None, inference: bool = False):
        """
        If amp_autocast=True and running on CUDA, we enter an fp16 autocast region.
        Numerically sensitive kernels (eigh/Sinkhorn/graph build) already run in fp32 internally.
        """
        if self.amp_autocast and X.is_cuda:
            ac = torch.amp.autocast('cuda', dtype=torch.float16)
        elif self.amp_autocast and not X.is_cuda:
            ac = torch.amp.autocast('cpu', dtype=torch.float16)
        else:
            ac = nullcontext()

        with ac:
            # Precompute or use cached operators (built in fp32 inside helper)
            ...
            for layer in self.layers:
                # diffusion on normalized features
        ...
        if self.training and not (inference or self.inference):
            # Gumbel–Sinkhorn runs stably in fp32 internally; returns same dtype as logits
...
if __name__ == "__main__":
    ...
    model = create_enhanced_ultrafast_sct_gnn(
        ...
        coord_input=True, tau=3.0, n_iter=10, noise_scale=0.05,
        logit_clamp=20.0, amp_autocast=True,   # enable AMP here
        attn_dropout=0.05, ff_dropout=0.1
    ).to(device)

    print(f"Model created with {sum(p.numel() for p in model.parameters()):,} parameters")

    # Training pass (autocast active)
    model.train()
    P, Z = model(X, A)
    ...
    # Equivariance test (kept in fp32 compute for comparison)
    with torch.no_grad():
